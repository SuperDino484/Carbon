...
namespace Carbon {
	...
	Window* Window::Create(const WindowData& args)
	{
		...
		return new WindowsWindow(args);
	}

	WindowsWindow::WindowsWindow(const WindowData& args)
		: Window(args)
	{
		Init();
	}
	...
	void WindowsWindow::SetEventCallback(WindowData::EventFnCallback callback)
	{
	...
	void WindowsWindow::Init()
	{
		...
		glfwSetWindowPosCallback(m_GLFWWindow, [](GLFWwindow* window, int xpos, int ypos) {
			WindowData& data = *(reinterpret_cast<WindowData*>(glfwGetWindowUserPointer(window)));
			WindowMovedEvent e((float)xpos, (float)ypos);
			data.EventCallback(e);
			});
